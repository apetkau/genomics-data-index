...
class MultipleProcessVcfVariantsTableProcessor(VcfVariantsTableProcessor):
    ...
    def process(self, sample_data: List[NucleotideSampleData], include_masked_regions: bool) -> Generator[
        ...
        def read_sample_data(sample_data: NucleotideSampleData) -> pd.DataFrame:
            return sample_data.read_sample_data_features(include_masked_regions=include_masked_regions)

        with mp.Pool(self._ncores) as pool:
            frame_vcf_masks = pool.imap_unordered(read_sample_data,
                                                  sample_data,
                                                  chunk_size)
